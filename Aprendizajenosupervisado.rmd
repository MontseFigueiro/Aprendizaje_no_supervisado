...
str(samsungData)
```
Veo cuantas observaciones hay para cada tipo de actividad. Veo las dos últimas columnas que tipo de dato tienen.
```{r}
...
```
Creo una tabla las filas serán el número de cluster y las columnas las actividades.
```{r}
table(kClust1$cluster,samsungData[,563])
```
El Cluster 1 tiene 855 observaciones walkup, el 2 653 laying, el 3 tiene 166 walkdown, así determino el nombre:
```{r}
